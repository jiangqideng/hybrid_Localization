...
//					if(RxBuf[4]==0xFF || s==10)	
 ...
 //
						...
						LED2=~LED2;
//改动
						SendPacket(TxBuf,11,9);					  //发送给下一地址的节点 （不同节点在这里目的地址需要改动）
						LED2=~LED2;
						i++;
				...
				else if(RxBuf[2]==7)					//本网络中只有这两种，不是广播信号就上上一节点发来的信号
				{
				LED1=~LED1;	
//改动
				SendPacket(RxBuf,11,9);					  //发送给下一地址的节点  （不同节点在这里目的地址需要改动）
				LED1=~LED1;
				Dispchar(i%256,5,48); 
				bh[RxBuf[3]-2]=1;
				}
