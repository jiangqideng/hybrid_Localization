...
//1<=page<=7,24<=seg<=120
****************************************************************/
/*void Dispchar(unsigned char dat,unsigned char page,unsigned char seg)			   //这个节点改成了只显示正的部分（为了观察更多数据）
{
...
void Dispchar2(unsigned char dat,unsigned char page,unsigned char seg)			   //这个节点改成了只显示正的部分（为了观察更多数据）
{
	unsigned char shiwei,gewei;
	shiwei=dat/10;
	gewei=dat-shiwei*10;
	writedigital(shiwei,page,seg);
	writedigital(gewei,page,seg-8);
}
...
//	draw_dot(23,0,82-xx);
	...
	if (xx==1 || xx==77)
	{  	if(yy==2)
		{draw_dot(39,temp,81-xx);
//		draw_dot(39,temp,82-xx);
//		draw_dot(39,temp,80-xx);
		}
		if(yy==3)
		{draw_dot(71,temp,81-xx);
//		draw_dot(71,temp,82-xx);
//		draw_dot(71,temp,80-xx);
		}
		if(yy==4)
		{draw_dot(135,temp,81-xx);
//		draw_dot(135,(yy+3)/8,82-xx);
//		draw_dot(135,(yy+3)/8,80-xx);
		}
		if(yy==53)
		{draw_dot(225,temp,81-xx);
//		draw_dot(225,temp,82-xx);
//		draw_dot(225,temp,80-xx);
		}
		if(yy==54)
		{draw_dot(226,temp,81-xx);
//		draw_dot(226,temp,82-xx);
//		draw_dot(226,temp,80-xx);
		}
		if(yy==55)
		{draw_dot(228,temp,81-xx);
//		draw_dot(228,temp,82-xx);
//		draw_dot(228,temp,80-xx);
		}
	}
	temp2=(yy+3)%8;
	if (yy>=5 && yy<=52)
	{
	draw_dot(1<<temp2,temp,81-xx);	
//	draw_dot(1<<temp2,temp,82-xx);
//	draw_dot(1<<temp2,temp,80-xx);	
	}
}
